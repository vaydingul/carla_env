...
def main(config):
	...
	mpc_module = mpc.MPC(config.device, 10, 40, ego_forward_model)
	...
	while not c.is_done:
		...
		if counter % 2 == 0:
			...
			control = mpc_module.optimize_action(current_state, target_state)
...
if __name__ == "__main__":
	...
	parser.add_argument("--ego_forward_model_path", type=str, default="pretrained_models/2022-09-27/16-21-41/ego_model_new.pt", help="Path to the forward model of the ego vehicle")
	parser.add_argument("--device", type=str, default="cpu", help="Device to use for the forward model")
