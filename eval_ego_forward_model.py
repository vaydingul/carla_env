...
def evaluate(fname, data, model, config):
	...
	rotation_loss_raw = np.rad2deg(np.mean(np.abs(vehicle_rotation[:-1, 1:2] - yaw_predicted)))
	...
	logger.info(savedir)
	logger.info(f"Location MAE: {np.mean(np.abs(vehicle_location[:-1, :2] - location_predicted))}")
	logger.info(f"Orientation Cos MAE: {np.mean(np.abs(np.cos(vehicle_rotation[:-1, 1:2]) - np.cos(yaw_predicted)))}")
	logger.info(f"Orientation Sin MAE: {np.mean(np.abs(np.sin(vehicle_rotation[:-1, 1:2]) - np.sin(yaw_predicted)))}")
...
def main(config):
	...
	logging.basicConfig(level=logging.INFO, filename=f"{config.save_dir}/eval_ego_forward_model.log", filemode="w")
	...
	for file in os.listdir(folder_name):
		if file.endswith(".npz"):
			...
			location_loss, rotation_loss, rotation_loss_raw = evaluate(file, data, model, config)
	...
	logger.info(f"Average Rotation Loss Raw: {np.mean(rotation_loss_raw_list)}")
	logger.info(config)
