...
wandb_links=("vaydingul/mbl/1gftiw9w") 
batch_sizes=(100)
...
FOLDER_NAME="old_world_forward_model_bev_evaluation_extensive_20Hz_with_metrics"
...
for ((j = 0; j < $length_checkpoint_numbers; j++)); do
	for ((i = 0; i < $length_wandb_links; i++)); do
		for ((k = 0; k < $length_thresholds; k++)); do
			...
			python3 eval_world_forward_model_old_bev.py \
