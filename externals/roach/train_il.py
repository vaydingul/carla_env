...
def main(cfg: DictConfig):
    ...
    if os.path.isdir(bc_dataset_path):
        ...
        subprocess.call(
            f"rsync -a --info=progress2 {bc_dataset_path} {cfg.cache_dir}", shell=True
        )
    else:
        try:
            api = wandb.Api()
            ...
            for i, data_hf in enumerate(all_hf):
                log.info(
                    f"{i+1}/{len(all_hf)}: Downloading {data_hf.name} to {cfg.cache_dir}"
                )
    ...
    for path in cfg.dagger_datasets[:-1]:
        ...
        if len(list_bc_h5) < 0.2 * n_ep_bc:
            log.warning(
                f"Not enough BC episode left ({len(list_bc_h5)}), discard dagger dataset {path}."
            )
            # break for loop
            break
        else:
            # pre delete 10% to save disk space
            n_episode_pre_delete = int(
                min(n_ep_bc * 0.1, len(list_bc_h5) - 0.2 * n_ep_bc)
            )
        ...
        if os.path.isdir(path):
            ...
            subprocess.call(
                f"rsync -a --info=progress2 {path} {cfg.cache_dir}", shell=True
            )
        ...
        while delete_size < dagger_size:
            if len(list_bc_h5) < 0.2 * n_ep_bc:
                log.warning(
                    f"Not enough BC episode left ({len(list_bc_h5)}), stop deleting expert dataset."
                )
                # break while loop
                break
            else:
        ...
        bc_size = sum(f.stat().st_size for f in list_bc_h5)
        log.info(
            f"BC dataset: {len(list_bc_h5)} episodes, {bc_size/1024**3:.2f}G. "
            f"Dagger dataset {path}: {dagger_size/1024**3:.2f}G"
        )

    log.info(f"train_il.py working directory: {os.getcwd()}")

    assert len(cfg.driver) == 1, "Only one agent can be trained at one time."

    agent_name = next(iter(cfg.driver))
    cfg_agent = cfg.driver[agent_name]
