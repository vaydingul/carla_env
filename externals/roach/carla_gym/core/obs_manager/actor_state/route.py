...
class ObsManager(ObsManagerBase):
    ...
    def _define_obs_space(self):
        self.obs_space = spaces.Dict(
            {
                "lateral_dist": spaces.Box(
                    low=0.0, high=2.0, shape=(1,), dtype=np.float32
                ),
                "angle_diff": spaces.Box(
                    low=-2.0, high=2.0, shape=(1,), dtype=np.float32
                ),
                "route_locs": spaces.Box(
                    low=-5.0, high=5.0, shape=(self._route_steps * 2,), dtype=np.float32
                ),
                "dist_remaining": spaces.Box(
                    low=0.0, high=100, shape=(1,), dtype=np.float32
                ),
            }
        )
    ...
    def get_observation(self):
        ...
        for i in range(self._route_steps):
            ...
            wp_location_actor_coord = trans_utils.loc_global_to_ref(
                wp_location_world_coord, ev_transform
            )
            location_list += [wp_location_actor_coord.x, wp_location_actor_coord.y]

        # dist_remaining_in_km
        dist_remaining_in_km = (
            self._parent_actor.route_length - self._parent_actor.route_completed
        ) / 1000.0
