...
import externals.roach.carla_gym.core.task_actor.common.navigation.route_manipulation as gps_util
from externals.roach.carla_gym.core.task_actor.common.navigation.map_utils import (
    RoadOption,
)
...
class ObsManager(ObsManagerBase):
        ...
        self.obs_space = spaces.Dict(
            {
                ...
                "target_gps": spaces.Box(
                    low=-1e3, high=1e3, shape=(3,), dtype=np.float32
                ),
    ...
    def attach_ego_vehicle(self, parent_actor):
        ...
        sensor_transform = carla.Transform(
            location=sensor_location, rotation=sensor_rotation
        )
        self._gnss_sensor = self._world.spawn_actor(
            bp, sensor_transform, attach_to=parent_actor
        )
        weak_self = weakref.ref(self)
        self._gnss_sensor.listen(
            lambda gnss_data: self._parse_gnss(weak_self, gnss_data)
        )
    ...
    def get_observation(self):
        ...
        next_vec_in_global = gps_util.gps_to_location(
            next_gps
        ) - gps_util.gps_to_location(gnss_data)
        ...
        obs = {
            ...
            "imu": imu_data,
            "target_gps": np.array(
                [gps_point["lat"], gps_point["lon"], gps_point["z"]], dtype=np.float32
            ),
