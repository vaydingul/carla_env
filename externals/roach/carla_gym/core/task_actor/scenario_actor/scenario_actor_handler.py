from externals.roach.carla_gym.core.task_actor.common.task_vehicle import TaskVehicle
import numpy as np
...
class ScenarioActorHandler(object):
    ...
    def reset(self, task_config, hero_vehicles):
        ...
        for sa_id in actor_config:
            ...
            AgentClass = getattr(
                import_module(
                    "carla_gym.core.task_actor.scenario_actor.drivers." + module_str
                ),
                class_str,
            )
            self.scenario_agents[sa_id] = AgentClass(
                self.scenario_actors[sa_id],
                self.hero_vehicles,
