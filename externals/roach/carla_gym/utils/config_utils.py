...
def check_h5_maps(env_configs, obs_configs, carla_sh_path):
    ...
    txt_command = (
        f"Please run map generation script from project root directory. \n"
        f"\033[93m"
    ...
    for env_cfg in env_configs:
        ...
        if file_exists:
            ...
            txt_assert = (
                f"pixel_per_meter mismatch between h5 file ({hf_pixels_per_meter}) "
...
def load_entry_point(name):
    ...
    mod_name = mod_name.replace("agents", "externals.roach.drivers")
...
def load_obs_configs(agent_configs_dict):
    ...
    for actor_id, cfg in agent_configs_dict.items():
        obs_configs[actor_id] = json.load(open(cfg["path_to_conf_file"], "r"))[
            "obs_configs"
        ]
    return obs_configs
    ...
    for route in tree.iter("route"):
        ...
        for actor_type in ["ego_vehicle", "scenario_actor"]:
            ...
            for actor in route.iter(actor_type):
                ...
                waypoint_list = (
                    []
                )  # the list of waypoints that can be found on this route for this actor
