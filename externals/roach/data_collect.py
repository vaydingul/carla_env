...
from externals.roach.carla_gym.utils.expert_noiser import ExpertNoiser
from utils import saving_utils, server_utils
from drivers.rl_birdview.utils.wandb_callback import WandbCallback
...
def collect_single(
    ...
    longitudinal_noiser = (
        ExpertNoiser("Throttle", frequency=15, intensity=10, min_noise_time_amount=2.0)
        if noise_lon
        else None
    )
    lateral_noiser = (
        ExpertNoiser("Spike", frequency=25, intensity=4, min_noise_time_amount=0.5)
        if noise_lat
        else None
    )
    ...
    while not done["__all__"]:
        ...
        for actor_id, driver in driver_dict.items():
            ...
            if noise_lon:
                driver_control[actor_id], _, _ = longitudinal_noiser.compute_noise(
                    driver_control[actor_id],
                    obs[actor_id]["speed"]["forward_speed"][0] * 3.6,
                )
            if noise_lat:
        ...
        for actor_id, coach in coach_dict.items():
            ...
            if alpha_coach:
                if np.random.uniform(0, 1) > alpha_coach:
                    ...
                    coach_supervision[actor_id]["action"] = (
                        np.array(
                            [c_driver.throttle, c_driver.steer, c_driver.brake],
                            dtype=np.float32,
                        ),
                    )
                else:
        ...
        if coach_supervision == {}:
            im_rgb = data_writer.write(
                timestamp=timestamp,
                obs=obs,
                supervision=driver_supervision,
                reward=reward,
                control_diff=None,
            )
        else:
            ...
            for actor_id in coach_control.keys():
                ...
                c_driver = driver_control[actor_id]
                control_diff[actor_id] = np.abs(
                    np.array(
                        [
                            c_coach.throttle - c_driver.throttle,
                            c_coach.steer - c_driver.steer,
                            c_coach.brake - c_driver.brake,
                        ],
                        dtype=np.float32,
                    )
                )
        ...
        for actor_id, driver in driver_dict.items():
            if log_video:
                if actor_id in coach_supervision:
                    ...
                    diff_str = np.array2string(
                        control_diff[actor_id],
                        precision=2,
                        separator=",",
                        suppress_small=True,
                    )
                    info[actor_id]["terminal_debug"]["debug_texts"].append(
                        f"coach_a{action_str}"
                    )
                    info[actor_id]["terminal_debug"]["debug_texts"].append(
                        f"ct_diff{diff_str}"
                    )
            ...
            if done[actor_id] and (actor_id not in ep_stat_dict):
                ...
                if alpha_coach:
                    if actor_id in coach_dict:
                        ...
                        _ = agent.run_step(obs[actor_id], timestamp)
                        last_value = agent.supervision_dict["value"]
...
def main(cfg: DictConfig):
    ...
    for ev_id, ev_cfg in cfg.actors.items():
        ...
        cfg_driver = cfg.driver[ev_cfg.driver]
        ...
        if ev_cfg.coach is None:
            ...
            for k, v in OmegaConf.to_container(cfg.driver.cilrs.obs_configs).items():
            ...
            cfg_coach = cfg.driver[ev_cfg.coach]
    ...
    ep_state_buffer_json = (
        f"{hydra.utils.get_original_cwd()}/outputs/ep_stat_buffer_{env_idx}.json"
    )
    ...
    if cfg.actors.hero.driver == "cilrs":
        ckpt_project = cfg.driver.cilrs.wb_run_path.split("/")[1]
        ckpt_run_id = cfg.driver.cilrs.wb_run_path.split("/")[2]
    ...
    env = gym.make(
        ...
        port=cfg.port,
        seed=cfg.seed,
        no_rendering=cfg.no_rendering,
        **env_setup["env_configs"],
    )
    ...
    for task_idx in range(ckpt_task_idx, n_episodes_per_env):
        ...
        while True:
            ...
            data_writer = saving_utils.DataWriter(
                dataset_dir / f"{run_name}.h5", cfg.ev_id, im_stack_idx
            )
            ...
            log.info(
                f"Start episode {run_name}, noise_lon={noise_lon}, noise_lat={noise_lat}, {env_setup}"
            )
            (
                valid,
                list_debug_render,
        ...
        if cfg.log_video:
            ...
            encoder.close()
            wandb.log(
                {f"video/debug_{run_name}": wandb.Video(debug_video_path)},
                step=idx_episode,
            )
            if cfg.actors.hero.driver != "cilrs":
                data_video_path = (video_dir / f"data_{run_name}.mp4").as_posix()
                encoder = ImageEncoder(
                    data_video_path, list_data_render[0].shape, 30, 30
                )
                for im in list_data_render:
                    encoder.capture_frame(im)
                encoder.close()
                wandb.log(
                    {f"video/data_{run_name}": wandb.Video(data_video_path)},
                    step=idx_episode,
                )
            encoder = None
        ...
        wandb.log(
            {
                "time/total_step": timestamp["step"],
        ...
        dataset_size = (
            subprocess.check_output(["du", "-sh", dataset_dir])
            .split()[0]
            .decode("utf-8")
        )
