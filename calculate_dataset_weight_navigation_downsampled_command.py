...
def main(config):
    ...
    # -------------------------------- Weight Path ------------------------------- #
    weight_path = f"{config['dataset']['config']['data_path']}/{config['num_time_step_previous']}-{config['num_time_step_future']}-{config['dilation']}-{'-'.join(config['read_keys'])}-weights.pt"
