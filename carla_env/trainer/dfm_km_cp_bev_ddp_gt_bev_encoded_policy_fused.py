...
class Trainer(object):
                ...
                run.log(
                    {
                        ...
                        "train/green_light_cost": loss_dict["green_light_cost"],
                        "train/yellow_light_cost": loss_dict["yellow_light_cost"],
                        "train/red_light_cost": loss_dict["red_light_cost"],
                        "train/pedestrian_cost": loss_dict["pedestrian_cost"],
    ...
    def validate(self, run=None):
        ...
        with torch.no_grad():
            for i, (data) in enumerate(self.dataloader_val):
                ...
                green_light_cost_list.append(loss_dict["green_light_cost"].item())
                yellow_light_cost_list.append(loss_dict["yellow_light_cost"].item())
                red_light_cost_list.append(loss_dict["red_light_cost"].item())
                pedestrian_cost_list.append(loss_dict["pedestrian_cost"].item())
    ...
    def render(self, i, world_future_bev_predicted, cost, action_pred, action_gt):
        ...
        if self.debug_render and self.num_time_step_future > 1:
                ...
                for m in range(self.num_time_step_future - 1):
                    ...
                    bev = cv2.cvtColor(
                        BirdViewProducer.as_rgb_with_indices(
                            np.transpose(bev, (1, 2, 0)), [0, 5, 6, 8, 9, 9, 10, 11]
                        ),
                        cv2.COLOR_BGR2RGB,
                    )
            ...
            cv2.imwrite(str(path_ / f"{i}_car.png"), self.canvas_car)
            cv2.imwrite(str(path_ / f"{i}_side.png"), self.canvas_side)
            cv2.imwrite(str(path_ / f"{i}_light.png"), self.canvas_light)
