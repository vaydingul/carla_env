...
class PolicyCILRS(nn.Module):
	def __init__(self, config):
		super(PolicyCILRS, self).__init__()
		...
		self.measurement_decoder = nn.Sequential(
		...
		branch_network_unit = Encoder(
			input_size=self.hidden_size,
			output_size=self.action_size,
			layers=self.branch_layers,
			dropout=self.dropout,
		)

		self.branch_network = Branch(
			module=branch_network_unit,
	...
	def build_from_config(self):
		...
		self.branch_layers = self.config["branch_layers"]
		self.branch_dropout = self.config["branch_dropout"]
