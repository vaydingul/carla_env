...
def build_vanilla_frame_encoder(input_size, output_size, layers, dropout):
    if layers == 3:
...
def build_rn18_frame_encoder(input_size, output_size):
    rn18 = torchvision.models.resnet.resnet18(pretrained=False)
    # remove pooling and fc layer
    f_encoder = nn.Sequential(*list(rn18.children())[:-2])
    f_encoder[0] = nn.Conv2d(
        input_size,
        64,
        kernel_size=(7, 7),
        stride=(2, 2),
    ...
    linear = nn.Linear(2048, output_size)
    return nn.Sequential(f_encoder, View([-1]), linear)


class Encoder(nn.Module):
    @staticmethod
    def build_frame_encoder(
        input_size, output_size, layers, dropout, hidden_size
    ):
        if layers < 4:
            return build_vanilla_frame_encoder(
                input_size, output_size, layers, dropout
            )
        elif layers == 18:
            return build_rn18_frame_encoder(
                input_size,
                hidden_size,
            )
        else:
            raise RuntimeError(f"unsupported # of layers: {layers}")
    ...
    def __init__(
        ...
        self.f_encoder = self.build_frame_encoder(
        ...
        if self.states:
            self.s_encoder = self.build_values_encoder(
        ...
        if self.a_size > 0:
            ...
            self.a_encoder = self.build_values_encoder(
                self.a_size,
                self.n_feature,
                self.hidden_size,
                self.dropout,
