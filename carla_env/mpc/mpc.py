...
class MPC(nn.Module):
	...
	def __init__(self, device,rollout_length, number_of_optimization_iterations, model):
		...
		self.action = torch.zeros((1, self.rollout_length, 3), device=self.device)
		#self.action[..., -1] = torch.randint_like(self.action[..., -1], 0, 2, dtype=torch.float32)
	...
	def reset(self, initial_guess = None):
		...
		if initial_guess is None:
			...
			action = torch.zeros((1, self.rollout_length, 3), device=self.device, dtype=torch.float32)
