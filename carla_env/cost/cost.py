...
class MaskedCostModule(nn.Module):
	...
	def forward(self, location, yaw, speed, location_next, yaw_next, speed_next, bev):
		# Create masks
		x , y = location_next[..., 0] - location[..., 0], location_next[..., 1] - location[..., 1]
		yaw_ = yaw_next - yaw
		speed_ = speed_next

		mask_car, mask_side = self.create_mask(self.image_width, self.image_height, self.pixels_per_meter, x, y, yaw_, speed_, self.vehicle_width, self.vehicle_length)
		
		car_channel = torch.where(bev[..., -1] == 1, torch.tensor(1), torch.tensor(0))
		offroad_channel = torch.where(bev[..., -1] == 2, torch.tensor(1), torch.tensor(0))
	...
	def create_masks(self, nx, ny, pixels_per_meter, x, y, yaw, speed, vehicle_width, vehicle_length):
