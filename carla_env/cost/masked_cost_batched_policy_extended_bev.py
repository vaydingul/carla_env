...
class Cost(nn.Module):
    ...
    def forward(
        ...
        decay_weight = torch.pow(
            self.decay_factor, torch.arange(
                0, speed_.shape[1], device=speed_.device).float()).reshape(
            1, -1, 1, 1, 1).repeat(
                speed_.shape[0], 1, bev.shape[2], 1, 1)
