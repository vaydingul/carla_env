...
def visualize_route(world, start = None, end = None):
	...
	for ix, (waypoint, road_option) in enumerate(route):
		...
		if ix <= 10:
			...
			world.debug.draw_string(waypoint.transform.location, 'o', color=WHITE, life_time=10000)

		elif ix >= len(route) - 10:

			world.debug.draw_string(waypoint.transform.location, 'o', color=BLACK, life_time=10000)
			
		else:

			world.debug.draw_string(waypoint.transform.location, 'o', color=GRAY, life_time=10000)
