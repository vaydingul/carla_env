...
def world_2_bev(loc, ego_loc, ego_yaw, image_height, image_width, pixels_per_meter = 18):
	...
	rotated_displacement_meters = np.array([[np.cos(ego_yaw), -np.sin(ego_yaw)], [np.sin(ego_yaw), np.cos(ego_yaw)]]) @ displacement_meters
	# Convert to pixels
	...
	bev_loc = displacement_pixels + np.array([image_width // 2, image_height // 2])
