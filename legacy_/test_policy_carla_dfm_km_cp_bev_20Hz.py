...
def render(num_time_step_future, world_future_bev_predicted, cost, action_pred):
    ...
    for k in range(1):
        for m in range(num_time_step_future - 1):
            ...
            mask_car = (
                ((mask_car - mask_car.min()) / (mask_car.max() - mask_car.min())) * 255
            ).astype(np.uint8)
            ...
            y1 = y2 + 20
            y2 = y1 + bev.shape[0]
            canvas[y1:y2, x1:x2] = cv2.addWeighted(bev, 0.5, mask_light, 0.5, 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--world_forward_model_wandb_link", type=str, default="vaydingul/mbl/bylewhod"
    )

    parser.add_argument("--world_forward_model_checkpoint_number", type=int, default=79)

    parser.add_argument(
        "--policy_model_wandb_link", type=str, default="vaydingul/mbl/3ohsztk8"
    )

    parser.add_argument("--policy_model_checkpoint_number", type=int, default=19)
